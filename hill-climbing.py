...
class MagicCube:
    ...
    def calculate_objective_value(self):
        error = (np.abs(self.row_sums - self.magic_number).sum() +
                 np.abs(self.col_sums - self.magic_number).sum() +
                 np.abs(self.layer_sums - self.magic_number).sum())
        
        for i in range(self.n):
            error += abs(self.cube[i, range(self.n), range(self.n)].sum() - self.magic_number)
            error += abs(self.cube[i, range(self.n), range(self.n-1, -1, -1)].sum() - self.magic_number)
            error += abs(self.cube[range(self.n), i, range(self.n)].sum() - self.magic_number)
            error += abs(self.cube[range(self.n), i, range(self.n-1, -1, -1)].sum() - self.magic_number)
            error += abs(self.cube[range(self.n), range(self.n), i].sum() - self.magic_number)
            error += abs(self.cube[range(self.n), range(self.n-1, -1, -1), i].sum() - self.magic_number)
...
class SteepestAscent:
    ...
    def get_neighbor(self):
        best_neighbor = None
        best_cost = self.cube.calculate_objective_value()
    ...
    def hill_climb(self):
        ...
        while True:
            ...
            if not neighbor or neighbor.calculate_objective_value() >= self.cube.calculate_objective_value():
                break
            self.cube = neighbor
            i += 1
...
class RandomRestartHillClimb:
    def __init__(self, initial_cube, max_restarts=100, goal_cost=0):
        self.cube = initial_cube  
    ...
    def hill_climb(self):
        ...
        restart = 0
        best_solution = self.cube
        best_cost = self.cube.calculate_objective_value()

        while restart < self.max_restarts and best_cost > self.goal:
            current_cost = self.cube.calculate_objective_value()
            print(f"Current Cost: {current_cost}, Restart: {restart}")
            ...
            if neighbor_cost < current_cost:
                self.cube = neighbor  
                if self.cube.calculate_objective_value() < best_cost:
                    best_solution = self.cube
                    best_cost = self.cube.calculate_objective_value()
            else:
                restart += 1
...
def run_with_custom_cube(custom_cube):
    ...
    rrhc = RandomRestartHillClimb(initial_cube, max_restarts=100, goal_cost=0)
